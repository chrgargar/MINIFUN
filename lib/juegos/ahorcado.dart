...
class _AhorcadoGameState extends State<AhorcadoGame> {
  ...
  void _selectNewWord() {
    ...
    // Inicializar pistas
    currentHints = ConstantesAhorcado.pistas[currentWord] ?? [];
    hintsAvailable = ConstantesAhorcado.pistasPorDificultad[widget.difficulty] ?? 1;
    usedHints = {};
    revealedHint = null;
  ...
  void _showHint() {
    if (usedHints.length >= hintsAvailable || currentHints.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(usedHints.length >= hintsAvailable 
            ? 'Ya no hay más pistas disponibles' 
            : 'No hay pistas para esta palabra'),
    ...
    setState(() {
      ...
      usedHints.add(availableIndex);
      revealedHint = currentHints[availableIndex];
    });

    // Mostrar un diálogo con la pista
    showDialog(
      context: context,
            ...
            builder: (context) {
              ...
              return Row(
                children: [
                  // Botón de pistas
                  Tooltip(
                    message: 'Pistas disponibles: ${hintsAvailable - usedHints.length}',
                    child: IconButton(
                      onPressed: _showHint,
                      icon: const Icon(Icons.lightbulb),
                      color: usedHints.length >= hintsAvailable 
                        ? Colors.grey 
                        : Colors.amber,
                    ),
                  ),
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          if (!widget.isSurvivalMode && !widget.isSpeedMode)
            Tooltip(
              message: 'Pistas: ${hintsAvailable - usedHints.length}/${hintsAvailable}',
              child: Row(
                children: List.generate(
                  hintsAvailable,
                  (index) => Icon(
                    index < (hintsAvailable - usedHints.length) 
                      ? Icons.lightbulb 
                      : Icons.lightbulb_outline,
                    color: index < (hintsAvailable - usedHints.length) 
                      ? Colors.amber 
                      : Colors.grey,
                    size: 20,
                  ),
